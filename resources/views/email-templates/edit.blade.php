        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Editar Template de Email</h3>
                </div>
                <form action="{{ route('admin.email-templates.update', $emailTemplate) }}" method="POST">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('key') is-invalid @enderror" id="key" name="key" value="{{ old('key', $emailTemplate->key) }}" required>
                                    @error('key')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('project_id') is-invalid @enderror" id="project_id" name="project_id">
                                        ...
                                        @foreach($projects as $project)
                                            <option value="{{ $project->id }}" {{ old('project_id', $emailTemplate->project_id) == $project->id ? 'selected' : '' }}>
                                                {{ $project->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('institution_id') is-invalid @enderror" id="institution_id" name="institution_id">
                                        ...
                                        @foreach($institutions as $institution)
                                            <option value="{{ $institution->id }}" {{ old('institution_id', $emailTemplate->institution_id) == $institution->id ? 'selected' : '' }}>
                                                {{ $institution->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('body_html') is-invalid @enderror" id="body_html" name="body_html" rows="10" required>{{ old('body_html', $emailTemplate->body_html) }}</textarea>
                            @error('body_html')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('body_text') is-invalid @enderror" id="body_text" name="body_text" rows="5">{{ old('body_text', $emailTemplate->body_text) }}</textarea>
                            @error('body_text')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" id="active" name="active" value="1" {{ old('active', $emailTemplate->active) ? 'checked' : '' }}>
