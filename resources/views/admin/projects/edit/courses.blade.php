...
<div class="container-fluid">
    ...
    <form method="POST"
          action="{{ route('admin.projects.edit.courses.update', $project) }}">
        @csrf
                ...
                <tbody>
                    @foreach($courses as $course)
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="courses[{{ $loop->index }}][active]"
                                        class="form-select form-select-sm"
                                        {{ $pivot ? '' : 'disabled' }}>
                                    <option value="1"
                                        {{ ($pivot && $pivot->active) ? 'selected' : '' }}>
                                        Sim
                                    </option>
                                    <option value="0"
                                        {{ ($pivot && !$pivot->active) ? 'selected' : '' }}>
                            ...
                            <td>
                                <input type="number"
                                       name="courses[{{ $loop->index }}][semester]"
                            ...
                            <input type="hidden"
                                   name="courses[{{ $loop->index }}][course_id]"
                                   value="{{ $course->id }}">
                        </tr>
                    @endforeach
                </tbody>
