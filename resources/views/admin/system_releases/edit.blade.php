...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-dark">Editar Notas da Versão</h1>
        <a href="{{ route('admin.system_releases.index') }}" class="btn btn-secondary shadow-sm">Voltar</a>
    </div>

    <div class="card shadow-sm border-0">
        <div class="card-body p-4">
            <form action="{{ route('admin.system_releases.update', $systemRelease) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-4">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="version" class="form-control" required value="{{ old('version', $systemRelease->version) }}" placeholder="Deve ser igual à tag do Git">
                        @error('version')
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="release_notes" class="form-control" rows="8" required placeholder="<ul><li>Nova funcionalidade X</li><li>Correção do erro Y</li></ul>">{{ old('release_notes', $systemRelease->release_notes) }}</textarea>
                        @error('release_notes')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
